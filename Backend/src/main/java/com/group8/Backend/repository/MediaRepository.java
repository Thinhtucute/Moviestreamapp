...
public interface MediaRepository extends JpaRepository<Media, Integer> {
        ...
        @Query("SELECT m FROM Media m ORDER BY m.viewCount DESC")
        List<Media> findTopByViewCount(int limit, Pageable pageable);

        default List<Media> findTopByViewCount(int limit) {
                return findTopByViewCount(limit, PageRequest.of(0, limit));
        }
}